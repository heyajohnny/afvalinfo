...
class CyclusAfval(object):
    ...
    def get_data(self, city, postcode, street_number):
        ...
        try:
            url = SENSOR_LOCATIONS_TO_URL["cyclus"][0].format(
            ...
            waste_dict["pbd"] = self.get_date_from_afvaltype(ophaaldata, 14)
            # find afvalstroom/3 = papier
            waste_dict["papier"] = self.get_date_from_afvaltype(ophaaldata, 3)
            # find afvalstroom/11 = textiel
            waste_dict["textiel"] = self.get_date_from_afvaltype(ophaaldata, 11)
